package com.dzpay.admin.common.dto.statis;

import com.dzpay.admin.common.error.exception.UnsupportTypeException;

/*
 * written : rojae
 * description : EDI_SQL.xml, DailyStatisDAO.java
 * 사용되는 EDI 파라미터 객체
 */

public class EDIDto {
	private String fr_ymd;
	private String to_ymd;
	private String site_cd;
	private String corp_id;
	private String card_cd;
	private String trade_status;
	private String description;
	
	/*
	 *  daily Payment		dp
	 *  daily Cancel		dc
	 *	month Payment		mp
	 *	month Cancel		mc
	 *	daily Meip			dm
	 *	daily Meip Cancel	dmc
	 */
	private String rtn_sts;			
	
	

	public String getFr_ymd() {
		return fr_ymd;
	}
	public void setFr_ymd(String fr_ymd) {
		this.fr_ymd = fr_ymd;
	}
	public String getTo_ymd() {
		return to_ymd;
	}
	public void setTo_ymd(String to_ymd) {
		this.to_ymd = to_ymd;
	}
	public String getSite_cd() {
		return site_cd;
	}
	public void setSite_cd(String site_cd) {
		this.site_cd = site_cd;
	}
	public String getCorp_id() {
		return corp_id;
	}
	public void setCorp_id(String corp_id) {
		this.corp_id = corp_id;
	}
	public String getCard_cd() {
		return card_cd;
	}
	public void setCard_cd(String card_cd) {
		this.card_cd = card_cd;
	}
	public String getTrade_status() {
		return trade_status;
	}
	public void setTrade_status(String trade_status) {
		this.trade_status = trade_status;
	}
	public String getDescription() {
		return description;
	}
	public void setDescription(String description) {
		this.description = description;
	}
	public String getRtn_sts() {
		return rtn_sts;
	}
	public void setRtn_sts(String rtn_sts) {
		this.rtn_sts = rtn_sts;
	}

	/*
	 * daily Payment		dp
	 * daily Cancel			dc
	 *	month Payment		mp
	 *	month Cancel		mc
	 *	daily Meip			dm
	 *	daily Meip Cancel	dmc
	 */
	public void convertColumn(String rtn_sts) throws Exception {
		if(this.rtn_sts.equals("")) {
			throw new UnsupportTypeException();
		}
		else if(this.rtn_sts.equals("dp")) {
			setDescription("Edi Today's Payments Amount for DuzonePay");
			setTrade_status("'ST01'");
		}
		else if(this.rtn_sts.equals("dc")) {
			setDescription("Edi Today's Cancellation Amount for DuzonePay");
			setTrade_status("'ST02'");
		}
		else if(this.rtn_sts.equals("mp")) {
			setDescription("Edi Month's Payments Amount for DuzonePay");
			setFr_ymd(getFr_ymd().substring(0, 6) + "01");
			setTrade_status("'ST01', 'ST20', 'ST30'");
		}
		else if(this.rtn_sts.equals("mc")) {
			setDescription("Edi Month's Cancellation Amount for DuzonePay");
			setFr_ymd(getFr_ymd().substring(0, 6) + "01");
			setTrade_status("'ST02','ST30'");
		}
		else if(this.rtn_sts.equals("dm")) {
			setDescription("Edi Today's Meip Amount for DuzonePay");
			setTrade_status("'ST20','ST30'");
		}
		else if(this.rtn_sts.equals("dmc")) {
			setDescription("Edi Today's Meip Cancellation Amount for DuzonePay");
			setTrade_status("'ST30'");
		}
		else
			throw new UnsupportTypeException();
	}
	
	public void clear() {
		this.fr_ymd = "";
		this.to_ymd = "";
		this.card_cd = "";
		this.corp_id = "";
		this.site_cd = "";
		this.trade_status = "";
		this.description = "";
	}
}
